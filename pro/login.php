...
if(isset($_POST['login'])){
    ...
    $result = $db->query($sql);
    //check if users account is activated of entered correct email password
    ...
    if ($row = $result->fetch_assoc()){
        ...
        if(isset($_POST['remember_me'])){
            setcookie("user_name" , $email , time()+60*5);
        }
        header("Location:profile.php");
        exit();
    }else {
        header("Location:login.php?err=" . urlencode("The account is not activated,login to your email and get the activation link"));
        exit();
    }
    }else {
        header("Location:login.php?err=" . urlencode("Invalid Email and password ,check and try again"));
        exit();
...
<body id="page-top" data-spy="scroll" data-target=".navbar-fixed-top" background="images/patterns/10.png">
<!--Navigation Bar-->
<nav class="navbar navbar-custom navbar-fixed-top" id="navbar" role="navigation">
  ...
  <!--placing menu to the right-->
    <ul class="nav navbar-nav navbar-right">
      ...
      <li class="active"><a href="login.php" id="text"><span class="glyphicon glyphicon-log-in"></span>  login</a></li>
