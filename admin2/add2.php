...
require 'config.php';
require 'db.php';
// capthures the information comin from the url
...
if (isset($_POST['clu'])) {
	...
	if (strlen($_POST['cert'])<=0) {
    	header("Location:add2.php?err=" . urlencode("Please select the Certification"));
        exit();
    }else if (strlen($_POST['cluster'])<=0) {
    	header("Location:add2.php?err=" . urlencode("Please select Cluster required"));
        exit();
    }else if(strlen($_POST['unit'])<=0){
        header("Location:add2.php?err=" . urlencode("Please select a Subject"));
        exit();
    }else if (strlen($_POST['grade'])<=0) {
    	header("Location:add2.php?err=" . urlencode("Please select Grade required"));
        exit();
    }else if (strlen($_POST['required'])<=0) {
    	header("Location:add2.php?err=" . urlencode("Please select if it's a Must, Optional or A Mean Grade"));
        exit();
    }
    else{
	...
	$sql9="insert into course_qualification (course_id,unit_code,score,clus_id,cert_id,required ) values('$subcat','$unit','$grade','$cluster','$cert','$required')";
	$red=$db->query($sql9);
	if ($red) {
		header("Location:add2.php?success=" . urlencode("Qualification added"));
	}else header("Location:add2.php?err=" . urlencode("Please Qualification already exists for this course or try again"));
	exit();
...
self.location='add2.php?cert=' + val ;
}*/
//used to reload the form once an input is inserted
function reload(form)
{
var val=form.cert.options[form.cert.options.selectedIndex].value;
self.location='add2.php?cert='+ val ;
...
self.location='add2.php?cert='+ val +'&cluster=' +val2 ;
}
function disableselect()
{
<?Php
if(isset($cert) and strlen($cert) > 0){
echo "document.f2.disabled = false;";}
else{echo "document.f2.disabled = true;";}
?>
}
//-->
...
<form method="post" action="add2.php" class="form-inline form-horizontal" name="f2" style="margin-top:80px;">
<div class="form-group">
<?php
$qu="SELECT * from course where course_id=$subcat";
$re=$db->query($qu);
$row2=$re->fetch_assoc();
...
<table width="100%" class="table table-striped table-condensed table-responsive" style="margin-top:50px;">
        ...
        <tbody>
        <?php
        $stmt ="SELECT * from course_qualification where course_id=$subcat";
        $str=$db->query($stmt);
